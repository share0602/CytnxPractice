...
'''
Construct one tensor of the partition of classical Ising model
Ref:https://esc.fnwi.uva.nl/thesis/centraal/files/f1825407968.pdf
equaition (14) in p.12
'''
Q = cytnx.Tensor([2, 2])
p_beta = np.exp(beta)
m_beta = np.exp(-beta)
...
'''
Ref:PHYSICAL REVIEW B 78, 205116 (2008) 
Especially Fig.4
'''
lnz = 0.0
for k in range(RGstep):
    ...
    ## Construct U1, V1
    ## Corresponds to S3,S1 in Fig.4a
    ...
    V1.permute_([1,2,0])
    ## Construct U2, V2
    ## Corresponds to S2,S4 in Fig.4a
    T.permute_([3,0,1,2])
    stmp, utmp, vtmp = cyx.xlinalg.Svd_truncate(T,chitemp)
    s_sqrt = stmp**0.5
    ...
    anet.PutCyTensors(['U1','V1','U2','V2'], [U1,V1,U2,V2]) ## Fig.4b
    T = anet.Launch(optimal = True)

lnz += T.Trace(0,2, by_label = True).Trace(1,3, by_label=True).item() / 2 ** (RGstep)
